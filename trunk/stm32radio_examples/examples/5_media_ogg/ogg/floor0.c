...
static ogg_int32_t vorbis_invsqlook_i(long a,long e){
...
#else
static ogg_int32_t vorbis_fromdBlook_i(long a){
  if(a>0) return 0x7fffffff;
  if(a<(-140<<12)) return 0;
...
static ogg_int32_t vorbis_coslook_i(long a){
...
static ogg_int32_t vorbis_coslook2_i(long a){
  int i=a>>COS_LOOKUP_I_SHIFT;
  int d=a&COS_LOOKUP_I_MASK;
...
static ogg_int32_t toBARK(int n){
  int i;
  for(i=0;i<54;i++) 
