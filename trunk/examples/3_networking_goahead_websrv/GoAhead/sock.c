 ...
 *	Add one to allow the user to write exactly SOCKET_BUFSIZ
 */
	if (ringqOpen(&sp->inBuf, SOCKET_BUFSIZ, SOCKET_BUFSIZ) == -1)
		goto __err2;
	if (ringqOpen(&sp->outBuf, SOCKET_BUFSIZ + 1, SOCKET_BUFSIZ + 1) == -1)
		goto __err1;
	if (ringqOpen(&sp->lineBuf, SOCKET_BUFSIZ, -1) == -1)
		goto __err0;

	return sid;

__err0:
	bfree(B_L, (char*)sp->inBuf.buf);
__err1:
	bfree(B_L, (char*)sp->outBuf.buf);
__err2:
